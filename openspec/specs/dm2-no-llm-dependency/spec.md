# DM2 No LLM Dependency

## Purpose
Guarantee that dm2 is a pure CLI tool with respect to LLMs across every surface it ships: the
distribution declares no LLM client library, no module under `src/dm2/` imports or calls one, no
documentation or help text instructs users to configure an LLM, and the configuration system
neither defaults nor persists `llm.*` settings. LLM capability belongs exclusively to the
external AI Agent that consumes dm2's structured `--json` output.
