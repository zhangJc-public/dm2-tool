...
dm2 SHALL define each workflow (propose, continue, new, ff, verify, onboard, bulk-archive) as a `WorkflowTemplate` dataclass in `src/dm2/core/templates/workflows/`, containing both `SkillTemplate` (SKILL.md content) and `CommandTemplate` (slash command .md content).
...
#### Scenario: All 7 workflows are registered
- **WHEN** `WORKFLOWS` is imported from `dm2.core.templates`
- **THEN** it SHALL contain exactly 7 entries: propose, continue, new, ff, verify, onboard, bulk-archive
