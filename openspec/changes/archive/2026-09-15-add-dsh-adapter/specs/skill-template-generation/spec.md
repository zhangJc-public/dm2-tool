# Skill Template Generation (delta)
...
- **AND** both fields SHALL be populated for every workflow even when the selected adapter emits no command file, so the template remains the single source for all targets

#### Scenario: Claude adapter output matches current SKILL.md
- **WHEN** the `ClaudeCodeAdapter`-generated SKILL.md is compared with the current `.claude/skills/dm2-*-workflow/SKILL.md`
...
- **AND** identity with the template text SHALL be an invariant of the Claude adapter only; an adapter whose target needs different phrasing SHALL render the body through `render_skill_body()` instead
