...
APPLY_SKILL = SkillTemplate(
    ...
    instructions="""Implement the architecture views planned in `/dm2:propose` by reading tasks.md and executing all pending view generation tasks.
...
APPLY_COMMAND = CommandTemplate(
    ...
    body="""Execute the implementation plan created by `/dm2:propose`. Reads tasks.md and generates all planned DoDAF views in dependency order.
...
def get_workflow_template() -> WorkflowTemplate:
    return WorkflowTemplate(
        workflow_id="apply",
        skill_dir="dm2-apply-workflow",
        command_file="apply.md",
        skill=APPLY_SKILL,
