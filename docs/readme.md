...
|------|------|------|
| 内置默认 | 代码内 | 开箱即用的默认值 |
| 用户配置 | `~/.config/dm2/config.yaml` | 跨项目共享（LLM 密钥、偏好） |
| 项目配置 | `.dm2/config.yaml` | 项目特定的视图和检查设置 |

### LLM Provider

生成视图时可选择 LLM provider（通过配置中的 `llm.provider` 项）：

| provider | 需要 | 配置示例 |
|----------|------|----------|
| `anthropic` | `ANTHROPIC_API_KEY` 环境变量 | `dm2 config -s llm.model=claude-opus-4-7` |
| `openai` | `OPENAI_API_KEY` 环境变量 | `dm2 config -s llm.provider=openai` |
...
dm2 analyze -d "等保三级整改，涉及边界防护、访问控制、审计日志" --llm
```
...
- **主要 6W** 和次要维度（含置信度）
- **提取的实体**（系统、组织、活动等）
- **推荐视图列表**（按优先级排序：必须/推荐/可选）

`--llm` 标志会启用 LLM 进一步补充缺失视图。
...
> **注意**：视图生成需要 LLM 配置。如未配置，输出将包含 `[需要 LLM]` 占位符。
...
执行完整的 DoDAF 架构开发流程（基于 LLM 驱动的融合 6 步框架）：
...
dm2 config -s llm.model=claude-opus-4-7      # 设置用户配置项
dm2 config -s llm.provider=openai            # 切换到 OpenAI
```

支持嵌套键（点分隔）：`llm.model`、`views.include_mermaid` 等。
...
# 4. 配置 LLM（如需要）
dm2 config -s llm.model=claude-sonnet-4-6
...
dm2 analyze -d "多部门协同的安全运营中心，跨三个数据中心" --llm
...
```bash
# 切换到 OpenAI
dm2 config -s llm.provider=openai
dm2 config -s llm.model=gpt-4o

# 关闭 Mermaid 图表
dm2 config -s views.include_mermaid=false

# 启用严格模式
dm2 config -s consistency.strict_mode=true
```
